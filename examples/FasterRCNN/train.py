...
class ResNetC4Model_RetinaNet(DetectionModel):
    def inputs(self):
        ret = [
    ...
    def build_graph(self, *inputs):
        ...
        retinanet_label_logits, retinanet_box_logits = retinanet_head('retinanet', featuremap, cfg.RPN.HEAD_DIM, cfg.RPN.NUM_ANCHOR)
        #******
        ...
        if is_training:
            # rpn loss
            retinanet_label_loss, retinanet_box_loss = retinanet_losses(
                anchors.gt_labels, anchors.encoded_gt_boxes(), retinanet_label_logits, retinanet_box_logits)
            ...
            total_cost = tf.add_n([
                retinanet_label_loss, retinanet_box_loss,
                mrcnn_loss, wd_cost], 'total_cost')
            ...
            final_boxes, final_labels = generate_retinanet_proposals(
...
def predict(pred_func, input_file):
    ...
    print('resultnum:'+str(len(results)))
    print(results[0])
    #results = detect_one_image(img, pred_func)
    final = draw_final_outputs(img, results)
    cv2.imwrite('results.png',final)
    viz = np.concatenate((img, final), axis=1)
    
    tpviz.interactive_imshow(viz)

def predict_cropimage(pred_func, input_file):
    num = args.number
    input_file = 'cropimages'
    ori_img = cv2.imread('test.jpg',cv2.IMREAD_COLOR)
    height = ori_img.shape[0]
    width = ori_img.shape[1]
    strides_h = height / num
    strides_w = width / num
    sum_results = []
    for i in range(num*num):
        #cimgfile = os.path.join(input_file,'img{}_{}.jpg'.format(num*num,i))        
        #img = cv2.imread(cimgfile, cv2.IMREAD_COLOR)
        
        h = int((i)/num)
        w = i % num
        img = ori_img[int(h*strides_h):int((h+1)*strides_h),int(w*strides_w):int((w+1)*strides_w),:]
        results = detect_one_image(img, pred_func)
        if len(results)>0:
            print('resultnum:'+str(len(results)))
        for r in results:
            r.box[0] = r.box[0] + w * strides_w
            r.box[1] = r.box[1] + h * strides_h
            r.box[2] = r.box[2] + w * strides_w
            r.box[3] = r.box[3] + h * strides_h
        sum_results = sum_results + results   
    #img = cv2.imread(input_file, cv2.IMREAD_COLOR)
    #results = detect_one_image(img, pred_func)
    print('sum resultnum:'+str(len(sum_results)))
    #results = detect_one_image(img, pred_func)
    final = draw_final_outputs(ori_img, sum_results)
    cv2.imwrite('results.png',final)
    viz = np.concatenate((ori_img, final), axis=1)
    tpviz.interactive_imshow(viz)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--predict_cropimage', help="Run prediction on cropimages. ")

    parser.add_argument('--config', help="A list of KEY=VALUE to overwrite those defined in config.py",
                        nargs='+')
    parser.add_argument('--number', help='crop size',type=int, default=3)
    ...
    if args.visualize or args.evaluate or args.predict or args.predict_cropimage:
        ...
        if args.predict or args.visualize or args.predict_cropimage:
        ...
        if args.visualize:
            ...
            elif predict_cropimage:
                predict_cropimage(pred, args.predict)
    else:
